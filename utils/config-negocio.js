...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '34bbede6-1576-4df4-986d-aed1d0a5e8c6'; // ID de HCNails
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '5926416658';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'hhechavarriacastillo743@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a HCNails!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'hcnails';
};
...
console.log('✅ config-negocio.js listo para HCNails');
